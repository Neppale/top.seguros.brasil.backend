static class PremiumGenerator
{
  ...
  public static decimal Generate(decimal vehicleValue, int id_cobertura, string dbConnectionString)
  {
    SqlConnection connection = new SqlConnection(dbConnectionString);

    // Recuperando valor da cobertura.
    decimal coberturaValue = connection.QueryFirst<decimal>("SELECT valor FROM Coberturas WHERE id_cobertura = @Id", new { Id = id_cobertura });
