...
{
  public static GetOneClientDto? Insert(Cliente cliente, SqlConnection connectionString)
  {
    try
    {
      ...
      var client = connectionString.QueryFirstOrDefault<GetOneClientDto>("SELECT * FROM Clientes WHERE email = @Email", new { Email = cliente.email });
      return client;
    }
