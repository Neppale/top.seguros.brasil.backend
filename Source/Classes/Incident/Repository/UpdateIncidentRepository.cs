static class UpdateIncidentRepository
{
  ...
  public static GetOneIncidentDto? Update(int id, Ocorrencia incident, SqlConnection connectionString)
  {
    try
    {
      ...
      var updatedIncident = connectionString.QueryFirstOrDefault<GetOneIncidentDto>("SELECT id_ocorrencia, data, local, UF, municipio, descricao, tipo, status, id_veiculo, id_cliente, id_terceirizado FROM Ocorrencias WHERE id_ocorrencia = @Id", new { Id = id });
