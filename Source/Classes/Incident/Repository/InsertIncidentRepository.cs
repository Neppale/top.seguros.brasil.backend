...
{
  public static GetOneIncidentDto? Insert(Ocorrencia incident, SqlConnection connectionString)
  {
    try
    {
      ...
      var createdIncident = connectionString.QueryFirstOrDefault<GetOneIncidentDto>("SELECT id_ocorrencia, data, local, UF, municipio, descricao, tipo, id_veiculo, id_cliente, id_terceirizado FROM Ocorrencias WHERE id_ocorrencia = (SELECT MAX(id_ocorrencia) FROM Ocorrencias)");
