static class InsertIncidentRepository
{
    public static async Task<GetIncidentByIdDto?> Insert(Ocorrencia incident, SqlConnection connectionString)
    {
        try
        {
            ...
            var createdIncident = await connectionString.QueryFirstOrDefaultAsync<GetIncidentByIdDto>("SELECT id_ocorrencia, data, local, UF, municipio, descricao, tipo, id_veiculo, id_cliente, id_terceirizado, status FROM Ocorrencias WHERE id_ocorrencia = (SELECT MAX(id_ocorrencia) FROM Ocorrencias)");
