static class GetUserReportRepository
{
    public static async Task<UserReport> Get(SqlConnection connectionString, int id)
    {
        ...
        {
            var fixedDate = SqlDateConverter.ConvertToDate(policy.data_inicio);
            var months = (DateTime.Now - fixedDate).TotalDays / 30;
            estimatedTotalGains += (decimal)((double)policy.premio * months);
        }
