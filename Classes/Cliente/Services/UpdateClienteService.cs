...
static class UpdateClienteService
{
  ...
  public static IResult Update(int id, Cliente cliente, string dbConnectionString)
  {
    ...
    try
    {
      var data = connectionString.Query("UPDATE Clientes SET email = @Email, senha = @Senha, nome_completo = @Nome, cnh = @Cnh, cep = @Cep, data_nascimento = @DataNascimento, telefone1 = @Telefone1, telefone2 = @Telefone2 WHERE id_cliente = @Id",
