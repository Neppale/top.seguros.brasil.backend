static class GetOneApoliceService
{
  ...
  public static IResult Get(int id, string dbConnectionString)
  {
    ...
    var data = connectionString.QueryFirstOrDefault<Apolice>("SELECT SELECT id_apolice, data_inicio, data_fim, premio, indenizacao, id_cobertura, id_usuario, id_cliente, id_veiculo, status from Apolices WHERE id_apolice = @Id", new { Id = id });
